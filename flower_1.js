...
bot.direction = function(game) {
    ...
    var baseDistance = bot.findDistance(game.myBot.pos, game.myBase.pos)
    ...
    if (game.myBot.pollen > 200) {
        task = "myBase"
    } else if (baseDistance * game.players.length == turnsLeft) {
        task = "myBase"
    } else if (game.mybot.pollen * 2 < enemyMostPollen.pollen) {
      task = "steal"  
    } else {task = "flower";}
    /* ~~This code decides how to do it ~~ */
    if (task == "flower"){
        console.log("seeking most flower pollen")
        myDir = bot.nextStep(game.myBot.pos, mostFlower.pos)
    } else if (task == "myBase"){
        console.log("returning pollen to base")
        myDir = bot.nextStep(game.myBot.pos, game.myBase.pos)
    } else if (task == "steal") {
        console.log("stealing from enemies...")
